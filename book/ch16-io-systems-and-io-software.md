...
![图 16-2：打印请求的数据路径](../assets/figures/ch16-printing-io-paths.svg)

图 16-2 先固定数据本身的路径：用户空间字符串进入内核缓冲区，再由打印机控制器逐步送向打印设备。程序控制 I/O、中断驱动 I/O 和 DMA I/O 的差别，不在于最终数据是否到达打印机，而在于等待、搬运和完成通知分别由谁承担。
...
**中断驱动 I/O** 把等待拆开。系统调用拷贝数据、初始化状态，启动第一个字符后就让出 CPU，调度器可以运行其他进程。之后，每次打印机需要继续服务时产生中断；如果输出尚未完成，中断处理程序把下一个字符写入数据寄存器，并更新计数和指针；如果计数已经归零，就唤醒等待打印完成的进程。于是等待设备的长时间空档被调度器拿去运行其他进程。

**DMA I/O** 再往前走一步。系统调用拷贝或固定好数据后设置 DMA 控制器，随后 CPU 可调度其他进程；整块传输由 DMA 控制器推进。DMA 完成后由控制器发出中断，内核再检查状态并唤醒用户进程，CPU 主要在设置和完成通知处介入。

把这三种方式放进同一条请求路径，可以用四个问题压缩记忆：系统调用阶段做了什么，谁推进数据，CPU 怎样等待，完成时怎样通知。

| 方式 | 系统调用阶段 | 数据推进者 | CPU 等待方式 | 完成通知 |
|---|---|---|---|---|
| 程序控制 I/O | 拷贝用户数据到内核缓冲区后，系统调用留在内核中继续执行 | CPU 轮询 READY，并把字符逐个写入打印机控制器数据寄存器 | CPU 基本原地等待，每个字符输出前都轮询 READY 状态 | 循环结束后系统调用返回 |
| 中断驱动 I/O | 拷贝数据并启动首字符输出后，阻塞当前进程并调度其他进程 | 中断处理程序在设备就绪时继续送下一个字符，并更新计数和指针 | CPU 在字符间隙运行其他进程，设备用中断叫回 CPU | 计数归零后，中断处理路径唤醒等待进程 |
| DMA I/O | 拷贝或固定数据后设置 DMA 控制器，然后调度其他进程 | DMA 控制器直接在主存和设备之间推进块传输 | CPU 不参与块内传输，只在设置和完成处介入 | DMA 完成中断到来后，内核处理状态并唤醒等待进程 |
...
![图 16-3：I/O 软件层次](../assets/figures/ch16-io-software-layers.svg)

图 16-3 的阅读顺序是双向的：I/O 请求沿层次向下传递，逐步接近硬件；I/O 应答沿层次向上返回，逐步恢复成上层能理解的结果。层次结构的好处不是把代码画得整齐，而是把职责边界固定下来。
...
| 设备驱动程序 | 把“打印这些字符”翻译成控制器命令，设置或访问设备寄存器并检查设备状态 | 驱动最了解具体控制器寄存器、状态位和命令格式 |
...
![图 16-4：缓冲技术对照](../assets/figures/ch16-buffering-techniques.svg)

图 16-4 把缓冲区放在设备和进程之间来看：设备可以先把数据交给内核缓冲区，进程稍后再处理；双缓冲则让两个缓冲区轮流承担“设备写入”和“进程处理”两种角色，使数据传输和计算处理更容易重叠。
...
单缓冲是在慢设备和快进程之间放一个临时暂存区；双缓冲则有两个暂存区轮流工作，一个接收设备数据，另一个被系统处理或交给用户。多缓冲进一步把暂存区组织成共享池，适合系统中同时有多条 I/O 流的场景。
...
![图 16-5：假脱机系统结构](../assets/figures/ch16-spooling-structure.svg)

图 16-5 中，输入设备经预输入程序把作业信息放入输入井；作业调度程序选择作业进入运行；运行结果经输出井和缓输出程序送往输出设备；井管理程序协调输入井、输出井与运行作业之间的数据。==假脱机让独占设备呈现共享效果==。
