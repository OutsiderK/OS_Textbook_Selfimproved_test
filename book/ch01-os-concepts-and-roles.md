...
这张分层图回答了一个容易被忽略的问题：操作系统为什么"两头不靠岸"。往上看，<u>用户态（user mode，CPU的一种运行模式，详见第2章）</u>的应用程序运行在它之上，把它当作可以请求服务的平台；往下看，它紧贴硬件，掌握着直接操作 CPU、存储器和 I/O 设备的特权。正因为它夹在中间，应用程序才不必、也不能越过它去直接摆弄硬件。
...
**回顾开篇问题** 回到最开始的那个问题——为什么一个 CPU 能同时服务浏览器、音乐播放器和文本编辑器？答案就在这四个特征里。并发让它们交替推进，共享让它们共用 CPU 和内存，异步让每个程序的推进时机不可预测，虚拟让每个程序都以为自己独占了整个机器。
