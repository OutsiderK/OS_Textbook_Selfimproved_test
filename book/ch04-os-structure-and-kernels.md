...
**如何理解这张图：** 看这张图时，注意三个区域：**上部分是用户程序**，这是应用代码执行的地方；**中间是系统调用接口** — 一条明确的分界线，用户程序不能越过它直接进入内核；**下部分是内核模块** — 文件系统、进程控制、设备驱动等，这些模块之间用箭头连接，表示它们可以相互直接调用。
...
**如何理解这张图：** 观察这张图时，留意两个关键区域和它们之间的关系。**上半部是用户态** — 包括应用进程和各个服务进程（文件系统、设备驱动、虚拟存储管理、安全服务），它们并排存在；**下半部是核心态** — 微内核只保留最基本的机制。关键是**看箭头**：消息流向是怎样的？应用和服务之间不直接调用，所有请求都必须通过微内核路由。这与整体式的"模块间直接相互调用"形成了鲜明对比。
...
**如何理解这张图：** 这张图将两种模型对比展示。**左边（UNIX 模型）**：应用进程在上方，用户代码执行；当调用系统调用时，注意内核代码在**同一个方框内**执行——这表示内核借用了应用进程的身份，运行在同一个执行上下文里。**右边（Windows/Minix 模型）**：应用进程、内核、和服务进程是**三个独立的方框**，注意它们之间用箭头连接，表示消息在它们之间流动，而不是代码的直接执行。
